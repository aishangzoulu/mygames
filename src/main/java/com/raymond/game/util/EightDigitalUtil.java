...
算法要点：隐式图上的bfs
 */
public class EightDigitalUtil {
    //用于状态到0~9!中数字的hash算法
    private static int[] factorial = {1, 1, 2, 6, 24, 120, 720, 5040, 40320};
    //标注哪些状态已经访问，判重
    ...
    public String getResult(String id) {
        //标注哪些状态已经访问，判重
        stateArr = new boolean[362879];
        //记录初始状态到某一状态的距离
        distArr = new int[362879];
        //记录初始状态到某一状态的路径
        pathArr = new String[362879];
        String result = "";
        //用长度为9的一维数组表示状态
        int[] beginState = new int[9];
        for (int i = 0; i < beginState.length; i++) {
            beginState[i] = Integer.parseInt(id.charAt(i) + "");
        }
        ...
        while (state_list.size() > 0) {
            ...
            if (success(state, endState)) {
                ...
                result = pathArr[state_hash(endState)].replaceAll(
                        "null", "");
     ...
     * @return
     */
    private int state_hash(int[] state) {
        int key = 0;
     ...
     * @return
     */
    private boolean success(int[] state, int[] endState) {
     ...
     * @return
     */
    private int getIndex_0(int[] state) {
        int index_0 = -1;
        for (int i = 0; i < state.length; i++) {
            if (state[i] == 0) {
                index_0 = i;
